...
function filter(cat) {
    ...
    document.querySelectorAll('.chip').forEach(btn => {
        ...
        if (
            (cat === 'Alle' && btn.innerText === 'Alle steder') ||
            (cat === 'IRN' && btn.innerText === '🛡️ Kun IRN') ||
            (cat === 'Top10' && btn.innerText === 'Nokta. topp 10') ||
            (cat === 'Trender' && btn.innerText.includes('Trender')) ||
            (cat === 'FastFood' && btn.innerText === 'Fast Food') ||
            (cat === 'Midtosten' && btn.innerText === 'Tyrkisk & Midtøsten') ||
            (cat === 'Asian' && btn.innerText === 'Asian & Wok') ||
            (cat === 'Indisk' && btn.innerText.includes('Indisk')) ||
            (cat === 'TakeAway' && btn.innerText === 'Take Away & Catering') ||
            (cat === 'Dessert' && btn.innerText === 'Dessert & Kos') ||
            btn.innerText.includes(cat)
        ) { btn.classList.add('active'); }
    });
    renderFeedAndMap();
}

// Kobler søkeboksen sammen med oppdateringen
...
// Sørger for oppstart etter en bitteliten forsinkelse så kartet rekker å henge med
